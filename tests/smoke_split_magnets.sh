...
echo "STL Master Split Connectors 4.0 magnets smoke test"
...
response="$(
  curl -sS -X POST \
    ...
    -F "magnet_size=6x2" \
    -F "magnet_diameter_mm=6" \
    -F "magnet_thickness_mm=2" \
...
printf '%s' "${job_json}" > "tests/results/split_magnets_${job_id}.json"
...
if split.get("success") is not True or split.get("split_mode") != mode:
    raise SystemExit("magnets split failed")
...
print(json.dumps({"job_id": data.get("job_id"), "integrated": connectors.get("integrated"), "magnet_size": connectors.get("magnet_size"), "reason": connectors.get("reason")}, ensure_ascii=False))
PY
...
echo "Split magnets smoke test passed."
