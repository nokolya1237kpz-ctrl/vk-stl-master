...
echo "STL Master auto orientation smoke test"

response="$(
  curl -sS -X POST \
    -F "file=@${REAL_MODEL}" \
    -F "operations=analyze,print_check,prepare_package,auto_orientation" \
...
printf '%s' "${job_json}" > "tests/results/auto_orientation_${job_id}.json"
...
auto = result.get("auto_orientation", {})
generated = {item.get("name") for item in result.get("generated_files", [])}
metrics = auto.get("metrics") or {}
dimensions = (metrics.get("bounding_box") or {}).get("dimensions") or {}
checks = {
    "auto_orientation.success": auto.get("success") is True,
    "final_download_url": bool(result.get("final_download_url")),
    "height.valid": float(metrics.get("height") or 0) > 0,
    "footprint.valid": float(metrics.get("footprint_area") or 0) > 0,
    "dimensions.valid": all(float(dimensions.get(key) or 0) > 0 for key in ("width", "depth", "height")),
}
if auto.get("no_change_needed"):
    checks["oriented_auto.not_generated"] = "oriented_auto.stl" not in generated
    checks["final_model.not_oriented_auto"] = result.get("final_model") != "oriented_auto.stl"
else:
    checks["oriented_auto.generated"] = "oriented_auto.stl" in generated
    checks["final_model.oriented_auto"] = result.get("final_model") == "oriented_auto.stl"
failed = [name for name, ok in checks.items() if not ok]
if failed:
    raise SystemExit("auto orientation contract failed: " + ", ".join(failed))
print(
    "OK auto orientation:",
    "job_id={};".format(data.get("job_id")),
    "candidate={};".format(auto.get("selected_candidate")),
    "no_change_needed={};".format(auto.get("no_change_needed")),
    "support_risk={};".format(metrics.get("support_risk")),
    "height={};".format(metrics.get("height")),
...
no_change_needed="$(printf '%s' "${job_json}" | json_value "data.get('result', {}).get('auto_orientation', {}).get('no_change_needed')")"
if [[ "${no_change_needed}" == "True" ]]; then
  if printf '%s\n' "${zip_contents}" | grep -qx "oriented_auto.stl"; then
    echo "ERROR: ZIP contains oriented_auto.stl even though no_change_needed=true" >&2
    printf '%s\n' "${zip_contents}" >&2
    exit 1
  fi
elif ! printf '%s\n' "${zip_contents}" | grep -qx "oriented_auto.stl"; then
  echo "ERROR: ZIP does not contain oriented_auto.stl" >&2
  printf '%s\n' "${zip_contents}" >&2
  exit 1
fi
...
echo "ZIP contents:"
printf '%s\n' "${zip_contents}"
echo "Auto orientation smoke test passed."
